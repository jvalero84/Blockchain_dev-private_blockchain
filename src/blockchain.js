...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const fiveMins = 5 * 60;
            if((currentTime - messageTime) >= fiveMins){ //Time elapsed exceeds 5 mins. Reject
                reject("The message was sent more than 5 minutes ago.")
            }
            if(!bitcoinMessage.verify(message, address, signature)){ //Message verification failed. Reject
                reject("The message verification has failed.")
            }
            let newBlock = new BlockClass.Block({"star":star,"owner":address});
            self._addBlock(newBlock)
                .then(resolve(newBlock))
                .catch(reject("Failed to add new Block to the chain"));
        });
    }
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
            let filteredChain = self.chain.filter(block => block.hash === hash);
            if(filteredChain.length > 0){
                resolve(filteredChain[0]);
            }else{
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise((resolve, reject) => {
            self.chain.forEach(async(chainElement)=> {
                if(chainElement.height>0){
                    let blockBody = await chainElement.getBData();
                    if(blockBody.owner === address){
                        stars.push(blockBody.star);
                     }
                }
            })
            resolve(stars);
